...
MiesOperator = R6Class("MiesOperator",
  public = list(
    ...
    operate = function(values, ..., context = list(inst = NULL)) {
      ...
      if (getOption("smashy.testing")) private$.primed_ps$assert_dt(values)
      ...
      if (self$endomorphism) {
        if (getOption("smashy.testing")) values = private$.primed_ps$assert_dt(values)[, match(ids, colnames(values), 0), with = FALSE]
        if (convert) {
          setDF(values)
        }
      }
